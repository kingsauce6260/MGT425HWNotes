...
# oversample houses with over 10 rooms #ifelse(test, yes, no)
s <- sample(row.names(housing.df), 5, prob = ifelse(housing.df$ROOMS>10, 0.9, 0.01))

housing.df[s,]

Challenge questions
•	Show the 10th row of columns 2 and 5
housing.df[10,c(2,5)]
•	Find the mean for Living area and tax value
mean(housing.df$LIVING.AREA)
